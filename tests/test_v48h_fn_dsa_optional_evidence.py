...
def test_v48h_adn_fn_dsa_absent_allowed_and_valid_optional_evidence_recorded() -> None:
...
def test_v49i5_adn_bundle_builder_emits_canonical_order_without_mutating_input(
...
def test_v49i5_adn_bundle_builder_preserves_empty_internal_shell_construction() -> None:
    bundle = build_signature_bundle(signatures=[])

    assert bundle["signatures"] == []


def test_v49i5_adn_bundle_builder_rejects_non_object_entry() -> None:
...
def test_v49i5_adn_bundle_builder_rejects_missing_or_unknown_algorithm(entry: dict) -> None:
...
def test_v49i5_adn_verifier_rejects_noncanonical_order_before_key_lookup_or_crypto(
    ...
    monkeypatch.setattr(
        "adn_v3.v4.signing.find_trusted_key",
...
def test_v49i5_adn_verifier_rejects_missing_required_before_key_lookup_or_crypto(
    algorithm_sequence: tuple[str, ...],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload_hash = signed_payload_hash(payload=unsigned_payload())
...
def test_v49i5_adn_verifier_completes_structural_preflight_before_key_lookup_or_crypto(
    defect: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload_hash = signed_payload_hash(payload=unsigned_payload())
...
def test_v48h_adn_valid_fn_dsa_cannot_rescue_required_failure(required_algorithm: str) -> None:
...
def test_v48h_adn_fn_dsa_valid_cannot_replace_missing_ml_dsa() -> None:
...
def test_v48h_adn_present_fn_dsa_invalid_duplicate_or_wrong_role_is_fatal() -> None:
    ...
    with pytest.raises(ValueError, match="trusted ADN key"):
...
def test_v48h_adn_present_fn_dsa_no_registry_key_is_fatal_not_absent() -> None:
    verdict = _valid_bundle_verdict(algorithms=(CLASSICAL_ED25519, ML_DSA, FN_DSA))
    trust_profile = build_test_trust_profile()
...
def test_v48h_adn_fn_dsa_profile_and_payload_binding_fail_closed() -> None:
...
def test_v48h_adn_standard_profile_is_authenticated_not_metadata_only() -> None:
    verdict = _valid_bundle_verdict(algorithms=(CLASSICAL_ED25519, ML_DSA, FN_DSA))
...
def test_v48h_adn_fn_dsa_bundle_verifier_records_profiles_order_stably() -> None:
    payload = unsigned_payload()
    payload_hash = signed_payload_hash(payload=payload)
...
def test_v48h_adn_standard_profile_allow_list_rejects_empty_profile() -> None:
    with pytest.raises(ValueError, match="standard_profile"):
        require_supported_standard_profile(algorithm=FN_DSA, standard_profile="")


def test_v48h_adn_signature_bundle_verifier_callback_fail_closed_contract() -> None:
    payload = unsigned_payload()
    payload_hash = signed_payload_hash(payload=payload)
    bundle = build_signature_bundle(
        signatures=[
